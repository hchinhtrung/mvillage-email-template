...
async def _capture_on_context(adapter, ctx, hotel_url, checkin, cfg, stealth):
    ...
    try:
        ...
        while waited < cfg.api_wait_timeout_s:
            ...
            if cap["req"] is not None and adapter.response_has_rooms(cap["resp_json"]):
...
async def _warm_camoufox(adapter, hotel_url, checkin, cfg, verbose):
    ...
    async with AsyncCamoufox(headless=cfg.headless) as browser:
        for attempt in range(2):
            ...
            got_rooms = adapter.response_has_rooms(cap.get("resp_json"))
            if got_req and got_rooms:
                cap["engine"] = "camoufox"
                cap["impersonate"] = pick_impersonate(cap.get("ua"), "camoufox", cfg.impersonate)
                if verbose:
                    print(f"  ✅ warm OK (camoufox) | impersonate={cap['impersonate']} rooms=True", flush=True)
                return cap
            if verbose:
                print(f"  ⟳ camoufox warm: req={got_req} rooms={got_rooms} → retry…", flush=True)
...
async def _warm_chromium(adapter, browser, hotel_url, checkin, cfg, stealth, verbose):
    ...
    for attempt in range(n):
        ...
        if got_req and got_rooms:
            ...
            if verbose:
                print(f"  ✅ warm OK (chromium {imp}) rooms=True", flush=True)
            return cap
        if verbose:
            print(f"  ⟳ chromium warm {imp}: req={got_req} rooms={got_rooms} → next profile…", flush=True)
        await asyncio.sleep(random.uniform(*cfg.warm_backoff))
...
async def warm_capture(adapter, hotel_url, checkin, cfg, chromium=None, stealth=None, verbose=False):
    ...
    for attempt in range(cfg.nav_attempts):
        ...
        try:
            ...
            while waited < cfg.api_wait_timeout_s:
                ...
                if adapter.response_has_rooms(cap.get("j")):

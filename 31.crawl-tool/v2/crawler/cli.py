...
def _cfg_from(args):
    ...
    if getattr(args, "hotels_parallel", None) is not None:
        cfg.hotels_parallel = max(1, int(args.hotels_parallel))
    if getattr(args, "weeks_parallel", None) is not None:
        cfg.weeks_parallel = max(1, int(args.weeks_parallel))
    return cfg
...
def build_parser():
    ...
    g.add_argument("--hotels-parallel", dest="hotels_parallel", type=int, default=None,
                   help="concurrent hotels in round 1 (default 1 = sequential)")
    g.add_argument("--weeks-parallel", dest="weeks_parallel", type=int, default=None,
                   help="concurrent weeks per hotel in browser fallback (default 2)")
